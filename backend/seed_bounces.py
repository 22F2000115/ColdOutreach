...
def seed():
    logger.info("Seeding database with sample bounced leads...")
    db = SessionLocal()
    ...
    if existing_user:
        ...
        db.commit()
        logger.info("Cleared previous demo user.")
    ...
    db.commit()
    db.refresh(smtp)
    logger.info("Created demo SMTP settings.")
    ...
    db.commit()
    db.refresh(campaign)
    logger.info("Created outreach campaign.")
    ...
    db.commit()
    logger.info(f"Successfully seeded {len(recipients)} recipients.")
    db.close()
